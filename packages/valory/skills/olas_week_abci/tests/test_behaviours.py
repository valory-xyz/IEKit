...
"""This package contains round behaviours of OlasWeekAbciApp."""
...
from packages.valory.skills.abstract_round_abci.behaviour_utils import make_degenerate_behaviour, BaseBehaviour
from packages.valory.skills.abstract_round_abci.test_tools.base import FSMBehaviourBaseCase
from packages.valory.skills.abstract_round_abci.test_tools.common import BaseRandomnessBehaviourTest
from packages.valory.skills.decision_making_abci.models import CeramicDBBase
from packages.valory.skills.olas_week_abci.behaviours import OlasWeekBaseBehaviour, OlasWeekRoundBehaviour, \
    OlasWeekTweetCollectionBehaviour, OlasWeekDecisionMakingBehaviour, OlasWeekRandomnessBehaviour, \
    OlasWeekOpenAICallCheckBehaviour, OlasWeekEvaluationBehaviour, OlasWeekSelectKeepersBehaviour
from packages.valory.skills.olas_week_abci.models import SharedState, OpenAICalls
from packages.valory.skills.olas_week_abci.rounds import Event, SynchronizedData, FinishedWeekInOlasRound
...
DUMMY_TWEETS_RESPONSE = {
    "data": [
        {"author_id": "1286010187325812739", "text": "tweet_text_1", "id": "1", "created_at": "2023-06-01T12:00:00Z", "conversation_id": "1001"},
        {"author_id": "1286010187325812739", "text": "tweet_text_2", "id": "2", "created_at": "2023-06-01T12:05:00Z", "conversation_id": "1002"},
        {"author_id": "1286010187325812738", "text": "tweet_text_3", "id": "3", "created_at": "2023-06-01T12:10:00Z", "conversation_id": "1003"},
        {"author_id": "1286010187325812737", "text": "tweet_text_4", "id": "4", "created_at": "2023-06-01T12:15:00Z", "conversation_id": "1004"},
    ],
    "includes": {
        "users": [
...
DUMMY_WEEKLY_TWEETS = [
        {"author_id": "1286010187325812739", "text": "tweet_text_1", "id": "1", "created_at": "2023-06-01T12:00:00Z", "conversation_id": "1"},
        {"author_id": "1286010187325812739", "text": "tweet_text_2", "id": "2", "created_at": "2023-06-01T12:05:00Z", "conversation_id": "2"},
        {"author_id": "1286010187325812738", "text": "tweet_text_3", "id": "3", "created_at": "2023-06-01T12:10:00Z", "conversation_id": "3"},
        {"author_id": "1286010187325812737", "text": "tweet_text_4", "id": "4", "created_at": "2023-06-01T12:15:00Z", "conversation_id": "4"},
    ]
# WEEK, YEAR = OlasWeekEvaluationBehaviour.get_week_number_and_year()
# DUMMY_MOST_VOTED_TWEETS_DATA = [f"Week {WEEK} '{YEAR} in Olas\n"
#                                 "\n"
#                                 "Highlights included:\n"
#                                 "tweet_text_1, tweet_text_2, tweet_text_3, tweet_text_4 \n"
#                                 "Stay tuned for more in Week 2... 🚀",
#                                 WEEK,
#                                 YEAR,]
...
class BaseBehaviourTest(FSMBehaviourBaseCase):
    ...
    def fast_forward(
        self, data: Optional[Dict[str, Any]] = None, ceramic_db: Optional[Any] = None, openai_calls:Optional[Any] = None
    ) -> None:
        ...
        #
        # if openai_calls:
        #     self.behaviour.initial_behaviour_cls.openai_calls = openai_calls
        print("PRINTING BEHAVIOUR IDS")
        print(self.behaviour.current_behaviour.auto_behaviour_id())
        print(self.next_behaviour_class.auto_behaviour_id())
        assert (
    ...
    def complete(self, event: Event) -> None:
        """Complete test"""
        print("I AM HERE COMPLETING")
        self.behaviour.act_wrapper()
        print("I AM HERE COMPLETING WRAPPER")
        self.mock_a2a_transaction()
        print("I AM HERE COMPLETING MOCK A2A")
        self._test_done_flag_set()
        print("I AM HERE COMPLETING DONE FLAG")
    ...
    # def mock_params(self, max_calls_reached) -> None:
    #     """Update skill params."""
    #     self.skill.skill_context.params.__dict__.update({"_frozen": False})
    #     self.skill.skill_context.params.max_calls_reached = max_calls_reached
...
class TestOlasWeekOpenAICallCheckBehaviour(BaseBehaviourTest):
    ...
    def test_run(self, test_case: BehaviourTestCase, kwargs: Any,) -> None:
        ...
        # Mock the params
        # class MockOpenAICalls:
        #     def reset(self, current_time: float) -> None:
        #         pass
        #
        #     def max_calls_reached(self) -> bool:
        #         return kwargs.get("max_calls_reached")
        #
        # params = cast(SharedState, self._skill.skill_context.params)
        # params.__dict__["_frozen"] = False
        # params.openai_calls = MockOpenAICalls()
        # self.fast_forward(test_case.initial_data)
        # self.behaviour.act_wrapper()
        # self.complete(test_case.event)

        with patch("packages.valory.skills.olas_week_abci.models.OpenAICalls") as MockOpenAICalls:
            openai_calls = MockOpenAICalls()
            openai_calls.max_calls_reached.return_value = kwargs.get("max_calls_reached")
            params = cast(SharedState, self._skill.skill_context.params)
            params.__dict__["_frozen"] = False
            # params.openai_calls = MockOpenAICalls()
...
class TestTweetCollectionBehaviour(BaseBehaviourTest):
    ...
    def test_run(self, test_case: BehaviourTestCase, kwargs: Any,) -> None:
        ...
        self.fast_forward(test_case.initial_data)
        now_ts = self.skill.skill_context.state.round_sequence._last_round_transition_timestamp
        start_time = (datetime.fromtimestamp(now_ts.timestamp()) - timedelta(days=7)).strftime("%Y-%m-%dT%H:%M:%SZ")
        ...
        [
            ...
            (
                BehaviourTestCase(
                    "Happy path",
                    initial_data=dict(performed_olas_week_tasks={
                        "openai_call_check": "no_allowance"
                    }),
...
class TestRandomnessBehaviour(BaseRandomnessBehaviourTest):
    ...
    # def test_all_keepers_blacklisted(
    #     self,
    # ) -> None:
    ...
    #         SynchronizedData(AbciAppDB(setup_data={})),
    #     )
    #     assert (
    #         cast(
    #             BaseBehaviour,
...
class TestOlasWeekSelectKeepersCeramicBehaviour(BaseSelectKeepersBehaviourTest):
    """Test SelectKeeperBehaviour."""

    select_keeper_behaviour_class = OlasWeekSelectKeepersBehaviour
    next_behaviour_class = OlasWeekDecisionMakingBehaviour
