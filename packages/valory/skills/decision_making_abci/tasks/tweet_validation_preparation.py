...
import json
from eth_account.messages import encode_structured_data, _hash_eip191_message, encode_defunct
from eth_account.account import Account
from eth_utils.exceptions import ValidationError
from packages.valory.protocols.ledger_api import LedgerApiMessage
from aea.exceptions import AEAEnforceError
from packages.valory.contracts.compatibility_fallback_handler import CompatibilityFallbackHandlerContract
...
class TweetValidationPreparation(TaskPreparation):
    ...
    def _post_task(self):
        ...
        filtered_tweets = [t for t in current_centaur["plugins_data"]["scheduled_tweet"]["tweets"] if t["request_id"] not in removal_ids]
    ...
    def clean_votes(self, tweet):
        ...
        for v in tweet["voters"]:
            is_valid = yield from self.validate_signature(message_hash, v.keys()[0], v.values()[0])
            if is_valid:
    ...
    def check_tweet_consensus(self, tweet: dict):
        ...
        self.behaviour.context.logger.info(f"Voting power is {total_voting_power} for tweet {tweet['text']}")
    ...
    def get_voting_power(self, address: str) -> Generator[None, None, int]:
        ...
        self.behaviour.context.logger.info(f"Voting power is {voting_power} for address {address}")
        return voting_power
